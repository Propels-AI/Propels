        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
                      ...
                      setLeadFormConfig((p: any) => {
                        ...
                        if (e.target.checked) {
                          if (!current.find((x) => x.key === f.key))
                            current.push({ key: f.key, type: f.type, label: f.label, required: f.required });
                        } else {
            ...
            <div className="space-y-2 pt-2 border-t border-border">
              <div className="text-xs text-muted-foreground font-sans">Custom Fields</div>
              {(() => {
                const list: any[] = Array.isArray(leadFormConfig.fields) ? leadFormConfig.fields : [];
                ...
                return customFields.map((field) => (
                  <div key={field.key} className="space-y-2 p-3 bg-muted/30 rounded-md">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-foreground font-sans">{field.label || field.key}</span>
                      <button
                        type="button"
                        onClick={() => {
                          setLeadFormConfig((p: any) => {
                            const current: any[] = Array.isArray(p.fields) ? [...p.fields] : [];
                            const filteredFields = current.filter((x) => x.key !== field.key);
                            return { ...p, fields: filteredFields };
                          });
                        }}
                        className="text-xs text-muted-foreground hover:text-destructive font-sans"
                      >
                        Remove
                      </button>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-xs text-muted-foreground mb-1 font-sans">Field Label</label>
                        <input
                          className="w-full bg-background border border-input text-foreground rounded-md px-3 py-1.5 text-sm font-sans focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-none"
                          value={field.label || ""}
                          onChange={(e) => {
                            setLeadFormConfig((p: any) => {
                              const current: any[] = Array.isArray(p.fields) ? [...p.fields] : [];
                              const fieldIndex = current.findIndex((x) => x.key === field.key);
                              if (fieldIndex >= 0) {
                                current[fieldIndex] = { ...current[fieldIndex], label: e.target.value };
                              }
                              return { ...p, fields: current };
                            });
                          }}
                      ...
                      <div>
                        <label className="block text-xs text-muted-foreground mb-1 font-sans">Field Type</label>
                        <Select
                          ...
                          onValueChange={(value) => {
                            setLeadFormConfig((p: any) => {
                              ...
                              const fieldIndex = current.findIndex((x) => x.key === field.key);
                              if (fieldIndex >= 0) {
                                current[fieldIndex] = { ...current[fieldIndex], type: value };
                              }
                              return { ...p, fields: current };
                            });
                          }}
                    ...
                    <div>
                      <label className="block text-xs text-muted-foreground mb-1 font-sans">Placeholder Text</label>
                      <input
                        className="w-full bg-background border border-input text-foreground rounded-md px-3 py-1.5 text-sm font-sans focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-none"
                        ...
                        onChange={(e) => {
                          setLeadFormConfig((p: any) => {
                            ...
                            const fieldIndex = current.findIndex((x) => x.key === field.key);
                            if (fieldIndex >= 0) {
                              current[fieldIndex] = { ...current[fieldIndex], placeholder: e.target.value };
                            }
                            return { ...p, fields: current };
                          });
                        }}
                    ...
                    <label className="flex items-center gap-2 text-xs text-foreground font-sans">
                      <input
                        type="checkbox"
                        className="accent-primary focus:ring-2 focus:ring-ring focus:ring-offset-2"
                        ...
                        onChange={(e) => {
                          setLeadFormConfig((p: any) => {
                            ...
                            const fieldIndex = current.findIndex((x) => x.key === field.key);
                            if (fieldIndex >= 0) {
                              current[fieldIndex] = { ...current[fieldIndex], required: e.target.checked };
                            }
                            return { ...p, fields: current };
                          });
                        }}
